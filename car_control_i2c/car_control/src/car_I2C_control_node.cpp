...
******************************/
#define MAX_ROBOT_SPEED  180
#define MIN_ROBOT_SPEED    0 
...
} m_robot_speed_float , m_current_robot_speed_float;
...
void *read_I2C_thread(void *pt)
{
		...
		while(1){
			while(num_bytes = read(file_I2C, received_data, 7) > 0){
				if((received_data[0] == 'R') && (received_data[2] == 'L') && (received_data[4] == 'B') && (received_data[6] == '*'))
				R_sonar_data = received_data[1];
				L_sonar_data = received_data[3];
				B_sonar_data = received_data[5];
				}
			sleep(100);
			}
}
...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
  {
	...
	write(file_I2C, protocal_test, 9); 
	
	printf("speed : %lf  angle : %d L_sonar : %d B_sonar : %d R_sonar : %d \n",m_current_robot_speed_float.data,m_robot_angle_int16.data,L_sonar_data,B_sonar_data,R_sonar_data);
